...
import { getCartCount, subscribeToCartUpdates } from "../lib/cart";
...
export default function NavbarComponent() {
  ...
  const [totalQuantity, setTotalQuantity] = useState(() => getCartCount());

  useEffect(() => {
    return subscribeToCartUpdates((count) => {
      ...
      <div className="relative mx-auto flex w-full max-w-7xl items-center justify-between gap-4 py-3 lg:py-4">
        ...
        <div className="z-10 flex items-center gap-2 sm:gap-3">
          {isLoggedIn ? (
            <div className="hidden items-center gap-2 sm:flex">
              <div
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 bg-white text-sm font-semibold text-gray-700 shadow-sm"
              >
                {userInitial}
              </div>
            </div>
          ...
          >
            ...
            <span
              className={`absolute -right-1 -top-1 flex min-h-4.5 min-w-4.5 items-center justify-center rounded-full bg-lime-500 px-1 text-[10px] font-semibold leading-none text-slate-950 tabular-nums transition-opacity ${
      ...
      {open && (
        <div id="mobile-nav" className="border-t border-gray-100 bg-white py-3 md:hidden">
          <div className="mx-auto flex w-full max-w-7xl flex-col gap-1">
            ...
            {isLoggedIn ? (
              <div className="rounded-xl px-3 py-3 text-sm font-medium text-gray-700">
